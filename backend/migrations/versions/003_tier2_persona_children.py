...
down_revision: Union[str, None] = "002_tier1_persona"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("work_model IN ('Remote', 'Hybrid', 'Onsite')", name="ck_workhistory_work_model"),
    )
    op.create_index("idx_workhistory_persona", "work_histories", ["persona_id"])
    op.create_index("idx_workhistory_order", "work_histories", ["persona_id", "display_order"])
    ...
    op.create_table(
        ...
        sa.CheckConstraint("proficiency IN ('Learning', 'Familiar', 'Proficient', 'Expert')", name="ck_skill_proficiency"),
    )
    op.create_index("idx_skill_persona", "skills", ["persona_id"])
    op.create_index("idx_skill_name", "skills", ["persona_id", "skill_name"], unique=True)
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.ForeignKeyConstraint(["persona_id"], ["personas.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["related_job_id"], ["work_histories.id"], ondelete="SET NULL"),
    )
    op.create_index("idx_achievementstory_persona", "achievement_stories", ["persona_id"])
    ...
    op.create_table(
        ...
        sa.Column("sample_phrases", JSONB(), nullable=True, server_default=sa.text("'[]'::jsonb")),
        sa.Column("things_to_avoid", JSONB(), nullable=True, server_default=sa.text("'[]'::jsonb")),
        sa.Column("writing_sample_text", sa.Text(), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.ForeignKeyConstraint(["persona_id"], ["personas.id"], ondelete="CASCADE"),
    )
    op.create_index("idx_voiceprofile_persona", "voice_profiles", ["persona_id"], unique=True)
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.ForeignKeyConstraint(["persona_id"], ["personas.id"], ondelete="CASCADE"),
        sa.CheckConstraint("filter_type IN ('Exclude', 'Require')", name="ck_custom_non_negotiable_type"),
    )
    op.create_index("idx_customnonneg_persona", "custom_non_negotiables", ["persona_id"])
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.ForeignKeyConstraint(["persona_id"], ["personas.id"], ondelete="CASCADE"),
        sa.CheckConstraint("embedding_type IN ('hard_skills', 'soft_skills', 'logistics')", name="ck_personaembedding_type"),
    )
    op.create_index("idx_personaembedding_persona", "persona_embeddings", ["persona_id"])
    op.create_index("idx_personaembedding_type", "persona_embeddings", ["persona_id", "embedding_type"])
    ...
    op.add_column("personas", sa.Column("original_resume_file_id", sa.UUID(), nullable=True))
    ...
    op.create_table(
        ...
        sa.Column("included_jobs", JSONB(), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("job_bullet_selections", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("job_bullet_order", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("included_education", JSONB(), nullable=True, server_default=sa.text("'[]'::jsonb")),
        sa.Column("included_certifications", JSONB(), nullable=True, server_default=sa.text("'[]'::jsonb")),
        sa.Column("skills_emphasis", JSONB(), nullable=True, server_default=sa.text("'[]'::jsonb")),
        ...
        sa.ForeignKeyConstraint(["persona_id"], ["personas.id"], ondelete="CASCADE"),
        sa.CheckConstraint("status IN ('Active', 'Archived')", name="ck_baseresume_status"),
    ...
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('Pending', 'Resolved')", name="ck_personachangeflag_status"),
        sa.CheckConstraint(
            "resolution IN ('added_to_all', 'added_to_some', 'skipped') OR resolution IS NULL",
            name="ck_personachangeflag_resolution",
        ),
    )
    op.create_index("idx_personachangeflag_persona", "persona_change_flags", ["persona_id"])
    op.create_index(
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("persona_id", "source_id", name="uq_usersourcepref_persona_source"),
    )
    op.create_index("idx_usersourcepref_persona", "user_source_preferences", ["persona_id"])
    ...
    op.create_index("idx_pollingconfig_persona", "polling_configurations", ["persona_id"], unique=True)
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("source_id", sa.UUID(), nullable=False),
        sa.Column("also_found_on", JSONB(), nullable=True, server_default=sa.text("'{\"sources\":[]}'::jsonb")),
        # Job details
        sa.Column("job_title", sa.String(255), nullable=False),
        sa.Column("company_name", sa.String(255), nullable=False),
        ...
        # Status and scoring
        sa.Column("status", sa.String(20), nullable=False, server_default=sa.text("'Discovered'")),
        ...
        sa.Column("previous_posting_ids", JSONB(), nullable=True, server_default=sa.text("'[]'::jsonb")),
        # Ghost detection
        sa.Column("ghost_score", sa.Integer(), nullable=False, server_default=sa.text("0")),
        ...
        # Check constraints
        sa.CheckConstraint("work_model IN ('Remote', 'Hybrid', 'Onsite') OR work_model IS NULL", name="ck_jobposting_work_model"),
        sa.CheckConstraint(
            "seniority_level IN ('Entry', 'Mid', 'Senior', 'Lead', 'Executive') OR seniority_level IS NULL",
            name="ck_jobposting_seniority",
        ),
        sa.CheckConstraint("status IN ('Discovered', 'Dismissed', 'Applied', 'Expired')", name="ck_jobposting_status"),
        sa.CheckConstraint("fit_score >= 0 AND fit_score <= 100 OR fit_score IS NULL", name="ck_jobposting_fit_score"),
        sa.CheckConstraint("stretch_score >= 0 AND stretch_score <= 100 OR stretch_score IS NULL", name="ck_jobposting_stretch_score"),
        sa.CheckConstraint("ghost_score >= 0 AND ghost_score <= 100", name="ck_jobposting_ghost_score"),
    )
    ...
    op.create_index("idx_jobposting_external", "job_postings", ["source_id", "external_id"])
