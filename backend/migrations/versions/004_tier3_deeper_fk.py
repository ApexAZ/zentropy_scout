...
down_revision: str | None = "003_tier2_persona_children"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["work_history_id"], ["work_histories.id"], ondelete="CASCADE"
        ),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["base_resume_id"], ["base_resumes.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
        ...
        # Check constraints
        sa.CheckConstraint(
            "status IN ('Draft', 'Approved', 'Archived')", name="ck_jobvariant_status"
        ),
    )
    ...
    op.create_table(
        ...
        ),  # Linked when Application created
        sa.Column("job_posting_id", sa.UUID(), nullable=False),
        sa.Column(
        ...
        # Check constraints
        sa.CheckConstraint(
            "status IN ('Draft', 'Approved', 'Archived')", name="ck_coverletter_status"
        ),
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "skill_type IN ('Hard', 'Soft')", name="ck_extractedskill_type"
        ),
    )
    op.create_index(
        "idx_extractedskill_jobposting", "extracted_skills", ["job_posting_id"]
    )
