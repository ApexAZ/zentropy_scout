...
| § | Task | Hints | Status |
|---|------|-------|--------|
| 1 | **Database indexes migration 011** — all 7 tenant indexes already existed (migrations 002–005) under abbreviated names. Renamed all 49 indexes across the schema to explicit `idx_{table}_{column}` convention for AI-searchability (REQ-014 §8.1). TDD: 8 tests (upgrade, downgrade, naming convention, auth indexes unchanged). Updated REQ-014 §8.1 to reflect actual state. | `db, tdd, commands, plan` | ✅ |
...
| 6 | **E2E multi-tenant tests** — Playwright tests verifying frontend correctly handles 404 responses for cross-tenant resources. Verify navigation shows only owned data. Test that accessing another user's resource URL shows error state, not data. 7 tests across 6 detail/list routes. | `playwright, e2e, tdd, plan` | ✅ |
