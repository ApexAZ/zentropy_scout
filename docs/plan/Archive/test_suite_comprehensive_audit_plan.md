...
| 5 | **Audit application & variant endpoints** — `test_api_variant_for_job`, `test_api_applications`, `test_api_base_resumes`, `test_api_cover_letters` (4 files) — all clean (CRUD, ownership, cross-tenant, credit gating, snapshot logic, timeline immutability, PDF render, archive/restore) | `plan, test` | ✅ |
| 6 | **Audit resume, chat & file endpoints** — `test_api_resume_templates`, `test_api_chat`, `test_api_files`, `test_api_bulk_operations` (4 files) — removed 6 tests (6 constructor mirrors #8 from SSE event schema tests in chat — ChatDoneEvent, ToolStartEvent, ToolResultEvent, DataChangedEvent; kept model_dump/to_sse serialization tests); other 3 files all clean | `plan, test` | ✅ |
| 7 | **Audit usage, preferences & generation endpoints** — `test_api_usage`, `test_api_user_source_preferences`, `test_export_endpoints`, `test_generation_endpoint` (4 files) — all clean (HTTP integration: balance/summary/history with monetary formatting, source preferences with cross-tenant isolation, PDF/DOCX export with content-type headers, resume generation with credit gating + IDOR prevention) | `plan, test` | ✅ |
...
| 3 | **Audit OAuth & sessions** — `test_magic_link_endpoints`, `test_oauth_endpoints`, `test_oauth_helpers`, `test_tenant_session` (4 files) — all clean (magic link send/verify with enumeration defense + single-use tokens, OAuth initiation/callback with PKCE + CSRF + CWE-209, state cookie JWT validation, tenant scoped session isolation) | `plan, test, security` | ✅ |
| 4 | **Audit admin services** — `test_admin_api`, `test_admin_routing_test`, `test_admin_schemas`, `test_admin_config_service`, `test_admin_management_service` (5 files) — removed 10 tests (8 response schema constructor mirrors #8: ModelRegistryResponse, PricingConfigResponse, TaskRoutingResponse ×2, FundingPackResponse, SystemConfigResponse, AdminUserResponse, CacheRefreshResponse; 2 AdminUserUpdate constructor mirrors #8: set_admin_true/false); other 4 files all clean (HTTP CRUD, routing dispatch, DB integration, cascade guards) | `plan, test` | ✅ |
...
| 3 | **Audit chat & onboarding agents** — `test_chat_agent`, `test_onboarding_utilities`, `test_onboarding_workflow`, `test_graph_invocation` (4 files). Removed 5 subsumed tests from test_graph_invocation (4 onboarding redirects + 1 error handling already covered by test_chat_agent). Other 3 files clean. | `plan, test` | ✅ |
| 4 | **Audit ghostwriter & scouter** — `test_ghostwriter`, `test_ghostwriter_prompts`, `test_scouter_agent`, `test_scouter_error_handling` (4 files). All clean — behavioral tests for trigger conditions, prompt building/sanitization, polling logic, error classification. | `plan, test` | ✅ |
| 5 | **Audit embedding & provider adapters** — `test_gemini_embedding_adapter`, `test_metered_provider`, `test_provider_errors`, `test_provider_name` (4 files). All clean — behavioral tests for API calls, batching, metering delegation, cross-provider dispatch, error taxonomy, abstract enforcement. | `plan, test` | ✅ |
| 6 | **Audit metering & billing** — `test_metering_integration`, `test_metering_models`, `test_metering_service`, `test_balance_gating`, `test_embedding_cost`, `test_usage_repository_summary` + `test_usage_repository` (7 files). Removed 6 tests: 2 redundant `test_default_created_at` from metering_models (subsumed by test_create_* which already assert created_at), 2 subsumed balance gating tests + 1 tautological disabled-metering test + 1 subsumed disabled-balance-check from metering_integration (all covered by test_balance_gating.py). Other 4 files clean. | `plan, test` | ✅ |
...
| 3 | **Audit selection & tailoring** — `test_growth_trajectory`, `test_base_resume_selection`, `test_story_selection`, `test_tailoring_decision`, `test_duplicate_story` (5 files). Removed 5 constructor mirrors #8 from tailoring_decision (TestTailoringSignal ×2, TestBulletSkillData ×1, TestTailoringDecision ×2 — all construct dataclasses and assert same values back). Other 4 files clean — behavioral tests calling real scoring/selection functions. | `plan, test` | ✅ |
| 4 | **Audit draft, reorder & review** — `test_auto_draft_threshold`, `test_bullet_reordering`, `test_user_review`, `test_regeneration` (4 files). Removed 18 tests: 3 subsumed boundary tests from auto_draft_threshold (TestAutoDraftBoundaryConditions duplicated assertions in TestQualifiesForAutoDraft); 6 constructor mirrors #8 from user_review (TestDismissResult ×2, TestFavoriteResult ×2, TestApplyResult ×2); 9 constructor/default-value mirrors from regeneration (7 constructor mirrors #8 + 1 default-value mirror #5 + 1 empty-tuple echo from TestRegenerationConfig, kept 2 approved frozen-test patterns). bullet_reordering clean (38 tests, all behavioral). | `plan, test` | ✅ |
| 5 | **Phase gate — full backend test suite + push** | `plan, commands` | ⬜ |
...
| 3 | **Audit persona lifecycle** — `test_persona_change`, `test_persona_embedding_generation`, `test_persona_sync`, `test_modification_limits`, `test_timeline_immutability` (5 files). All clean — 0 removals. Frozen immutability via replace() (persona_change), text building + embedding generation (persona_embedding_generation), DB integration change flags/resolution (persona_sync), guardrail violations per REQ-010 §4.4 (modification_limits), 405 enforcement per REQ-004 §9 (timeline_immutability). | `plan, test` | ✅ |
| 4 | **Audit voice, detection & expiration** — `test_voice_prompt_block`, `test_voice_validation`, `test_ghost_detection`, `test_expiration_detection` (4 files). Removed 4 constructor mirrors #8 from expiration_detection (TestExpirationDetectionResult: 4 tests constructing dataclass and asserting same values back — all combinations already exercised by TestGenerateExpirationMessage). Other 3 files clean — prompt block output/sanitization/truncation, blacklist detection, ghost scoring per REQ-003 §7. | `plan, test` | ✅ |
| 5 | **Audit golden set & credit repos** — `test_golden_set`, `test_golden_set_fixture`, `test_credit_repository`, `test_credit_repository_balance` (4 files). Removed 4 constructor mirrors #8 from golden_set (test_entry_accepts_all_valid_fields, test_entry_without_optional_notes, test_metadata_accepts_all_valid_fields, test_golden_set_holds_multiple_entries — all echo constructor values; get_entry + load + validation tests provide behavioral coverage). Other 3 files clean — fixture data quality, DB integration CRUD/pagination/isolation, atomic debit/credit/reconciliation/guards. | `plan, test` | ✅ |
| 6 | **Audit application workflow & misc** — `test_application_pin_archive`, `test_application_workflow`, `test_race_conditions` (3 files). All clean — 0 removals. pin/archive ORM integration (10), full application workflow pipeline with guards (40), security-critical race condition handling (3). All behavioral DB integration tests. | `plan, test` | ✅ |
| 7 | **Phase gate — full backend test suite + push** | `plan, commands` | ⬜ |
