...
| 5 | **Domain 4a: Agent & graph (delete-only)** — delete flagged tests from `test_scouter_error_handling.py` (4 DEL), `test_strategist_graph.py` (3 DEL), `test_ghostwriter_graph.py` (4 DEL), `test_onboarding_agent.py` (3 DEL). Note: `test_strategist_prompts.py` already deleted in Phase 1. Pure DELETE, but large files to read — use offset/limit to target specific test classes. 4 files, 14 deletions. | `tdd, commands, plan` | ⬜ |
...
| 7 | **Domain 5: Provider & adapter** — delete flagged tests from `test_provider_errors.py` (7 DEL), `test_claude_adapter.py` (3 DEL), `test_rate_limiting.py` (8 DEL), `test_ghostwriter.py` (3 DEL — same TriggerType enum tests as claude_adapter). Pure DELETE. 4 files, 21 deletions. | `tdd, commands, plan` | ⬜ |
| 8 | **Domain 6: API layer** — delete flagged tests from `test_api_responses.py` (6 DEL), `test_api_chat.py` (8 DEL), `test_api_pagination.py` (3 DEL), `test_api_filtering.py` (2 DEL). Pure DELETE. 4 files, 19 deletions. | `tdd, commands, plan` | ⬜ |
