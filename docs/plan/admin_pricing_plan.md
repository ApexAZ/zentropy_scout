...
| § | Task | Hints | Status |
|---|------|-------|--------|
| 1 | **Create ORM models for 5 admin tables + User extension (TDD)** — Five new models and one User column. **Read:** REQ-022 §4.1–§4.6 (table schemas), `backend/app/models/base.py` (Base, TimestampMixin pattern), `backend/app/models/usage.py` (pattern: UUID PK, CheckConstraint, Mapped types), `backend/app/models/user.py` (user model to extend), `backend/app/models/__init__.py` (export registry). **Create:** `backend/app/models/admin_config.py` (~150L) — 5 SQLAlchemy models: `ModelRegistry` (id UUID PK, provider VARCHAR(20), model VARCHAR(100), display_name VARCHAR(100), model_type VARCHAR(20) default 'llm', is_active BOOLEAN default TRUE, created_at, updated_at — TimestampMixin — UniqueConstraint(provider, model), CheckConstraint model_type IN ('llm', 'embedding')), `PricingConfig` (id UUID PK, provider, model, input_cost_per_1k NUMERIC(10,6), output_cost_per_1k NUMERIC(10,6), margin_multiplier NUMERIC(4,2), effective_date DATE, created_at, updated_at — TimestampMixin — UniqueConstraint(provider, model, effective_date), CheckConstraints on positive margin and non-negative costs), `TaskRoutingConfig` (id UUID PK, provider, task_type VARCHAR(50), model, created_at, updated_at — TimestampMixin — UniqueConstraint(provider, task_type)), `CreditPack` (id UUID PK, name VARCHAR(50), price_cents INTEGER, credit_amount BIGINT, stripe_price_id VARCHAR(255) nullable, display_order INTEGER default 0, is_active BOOLEAN default TRUE, description VARCHAR(255) nullable, highlight_label VARCHAR(50) nullable, created_at, updated_at — TimestampMixin — CheckConstraints price_cents>0, credit_amount>0), `SystemConfig` (key VARCHAR(100) PK — NOT UUID, value TEXT, description VARCHAR(255) nullable, created_at, updated_at — TimestampMixin). **Modify:** `backend/app/models/user.py` (~5L) — add `is_admin: Mapped[bool] = mapped_column(nullable=False, server_default=text("false"))`. **Modify:** `backend/app/models/__init__.py` (~10L) — import and export all 5 new models. **Create:** `backend/tests/unit/test_admin_config_models.py` (~25 tests) — Test: instantiate each model with valid data and verify attributes, check table names match spec, verify unique constraints exist (inspect `__table_args__`), verify CheckConstraints exist, verify SystemConfig uses key as PK not UUID, verify TimestampMixin on all 5 models, verify User.is_admin defaults to False. **Run:** `pytest tests/unit/test_admin_config_models.py -v` **Done when:** All 5 models instantiate correctly, User has is_admin, all constraints defined, models exported from `__init__.py`. | `tdd, db, plan` | 🟡 |
