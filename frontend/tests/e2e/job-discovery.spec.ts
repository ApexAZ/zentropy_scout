...
const JOB_001_ID = JOB_IDS[0]; // Standard scored job
const JOB_003_ID = JOB_IDS[2]; // High ghost risk
...
test.describe("Opportunities Table", () => {
	test("displays job list with titles and scores", async ({ page }) => {
		...
		// Ghost warning icon on job-003 (ghost_score=82)
		await expect(page.getByTestId(`ghost-warning-${JOB_IDS[2]}`)).toBeVisible();
	});
	...
	test("clicking job row navigates to detail", async ({ page }) => {
		...
		// Click the row containing the first job title
		await page.getByText("Senior Software Engineer").click();
		await expect(page).toHaveURL(`/jobs/${JOB_001_ID}`);
	...
	test("favorite toggle sends PATCH", async ({ page }) => {
		...
		// Click favorite toggle for job-001
		const patchPromise = page.waitForResponse(
			(res) =>
				res.url().includes(`/job-postings/${JOB_001_ID}`) &&
				res.request().method() === "PATCH",
		);
		await page.getByTestId(`favorite-toggle-${JOB_001_ID}`).click();
